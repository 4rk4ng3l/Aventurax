...
final class OoohBoi_Steroids { 
	...
	*/
	public function ooohboi_init_extensions( $ob_settings_options ) {
		...
		if( ! $ob_settings_options ) {
			...
			foreach( $extensions_array as $extension_class => $extension_token ) {
				if( ! isset( $ob_settings_options[ $extension_token ] ) ) {
					$ob_settings_options[ $extension_token ] = 'yes';
					update_option( 'steroids_for_elementor', $ob_settings_options );
				} else {
					if( $ob_settings_options[ $extension_token ] && 'yes' === $ob_settings_options[ $extension_token ] ) $extension_class::init();
				}
			}
	...
	*/
	public function ooohboi_register_styles() {
		...
		wp_register_style( 'ooohboi-steroids-styles', plugins_url( 'assets/css/main.css', __FILE__ ), NULL, self::VERSION . '24012022' );
	...
	*/
	...
	public function ooohboi_register_scripts_front() {
		...
		wp_register_script( 'ooohboi-steroids', plugins_url( 'assets/js/ooohboi-steroids-min.js', __FILE__ ), [ 'jquery' ], self::VERSION . '16012022b', true );
