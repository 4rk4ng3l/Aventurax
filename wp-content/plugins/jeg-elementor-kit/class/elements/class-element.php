...
class Element {
	...
	private function setup_hook() {
		...
		add_action( 'elementor/element/after_section_end', array( $this, 'enqueue_scripts' ), 10, 2 );
	}
	...
	public function add_widget_options( $element ) {
		...
		$element->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} > .elementor-widget-container' =>  ( ( defined( 'ELEMENTOR_VERSION' ) && version_compare( ELEMENTOR_VERSION, '3.5.0', '<' ) ) ? '-moz-transform: rotate({{SIZE}}deg); -webkit-transform: rotate({{SIZE}}deg); -o-transform: rotate({{SIZE}}deg); -ms-transform: rotate({{SIZE}}deg); transform: rotate({{SIZE}}deg);' : '--e-transform-rotateZ: {{SIZE}}deg;' ),
				),
			)
		);
	...
	public function add_section_options( $section, $args ) {
		...
		$section->add_control(
			...
			array(
				...
				'selectors'     => array(
					'{{WRAPPER}}.elementor-section.jkit-sticky-element--enabled.sticky-pinned' => 'transition: margin {{SIZE}}s, padding {{SIZE}}s, background {{SIZE}}s, box-shadow {{SIZE}}s',
					'{{WRAPPER}}.elementor-section.jkit-sticky-element--enabled' => 'transition: margin {{SIZE}}s, padding {{SIZE}}s, background {{SIZE}}s, box-shadow {{SIZE}}s',
				),
				'condition'     => array(
					'jkit_sticky_section'           => 'enabled',
					'jkit_sticky_background_color!' => '',
	 ...
	 */
	public function enqueue_scripts( $class, $section_id ) {
		if ( 'jkit_sticky_element_section' === $section_id ) {
