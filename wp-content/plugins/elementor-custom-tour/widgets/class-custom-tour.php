...
?>
		...
		//Definimos si el producto tiene o no acomodacion
		$hasAcomodacion = $product->get_attribute('acomodacion');

		//Establecemos cuales son los atributos por defecto
		$defaultAttributes = productGetDefaultAttributes();

		$defaultOrigen = (isset($defaultAttributes['pa_origen']) == true ? $defaultAttributes['pa_origen'] : null);
		$defaultAcomodacion = (isset($defaultAttributes['pa_acomodacion']) == true ? $defaultAttributes['pa_acomodacion'] : null);
		$defaultPago = (isset($defaultAttributes['pa_pago']) == true ? $defaultAttributes['pa_pago'] : null);
		$defaultFecha = (isset($defaultAttributes['pa_fecha']) == true ? $defaultAttributes['pa_fecha'] : null);
		...
		<div class="BoxTour">
			...
			<div class="rectangleYellowDiscount" id="porcentajeAhorro>
				<div class=" textYellowDiscount">Ahorra hasta un <?php echo $porcentaje_ahorro ?>%</div>
		</div>
		<div class="textWhiteDiscount" id="salePrice">Desde <span style="color:#007CBE" id="precioOferta"><?php echo '$' . $minimum_price . " COP" ?></span></div>
		<div class="textGrayDiscount" id="regularPrice"><?php echo '$' . $max_price . "COP" ?></span>
		</div>
		<div class="dropdownTour primerDropdown">
			...
			<div class="textoDropdown" id="txtOrigen">
				<?php
				if (isset($defaultOrigen)) {
					foreach ($origenes as $origen) {
						if (convertToSlug($origen) == $defaultOrigen) {
							echo $origen;
						}
					}
				} else {
		...
		<div class="dropdownTour segundoDropdown">
			...
			<div class="textoDropdown" id="txtPago">
				<?php
				if (isset($defaultPago)) {
					foreach ($pagos as $pago) {
						if (convertToSlug($pago) == $defaultPago) {
							echo $pago;
						}
					}
				} else {
		...
		<div class="dropdownTour tercerDropdown">
			...
			<div class="textoDropdown" id="txtAcomodacion">
				<?php
				if (isset($defaultAcomodacion)) {
					foreach ($acomodaciones as $acomodacion) {
						if (convertToSlug($acomodacion) == $defaultAcomodacion) {
							echo $acomodacion;
						}
					}
				} else {
					echo $acomodaciones[0];
				}
				?>
			</div>
			<div class="iconoDesplegable">
		...
		<div class="boxFechas">
			...
			<div class="boxFechasSel">
				<?php
				$first = true;
				if (isset($defaultFecha)) {
					foreach ($fechas as $fecha) {
						if (convertToSlug($fecha) == $defaultFecha) {
